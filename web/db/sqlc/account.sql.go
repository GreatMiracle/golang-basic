...
const updateAccountNotReturning = `-- name: UpdateAccountNotReturning :exec
UPDATE accounts
SET owner =$2
WHERE id = $1
`

type UpdateAccountNotReturningParams struct {
	ID    int64  `json:"id"`
	Owner string `json:"owner"`
}

func (q *Queries) UpdateAccountNotReturning(ctx context.Context, arg UpdateAccountNotReturningParams) error {
	_, err := q.db.Exec(ctx, updateAccountNotReturning, arg.ID, arg.Owner)
	return err
}
